...
namespace EWC.CustomWeapon.Properties.Effects
{
        ...
        ITriggerCallbackAgentSync
    {
        ...
        public override void TriggerApply(List<TriggerContext> contexts)
        {
            if (contexts.Count > 1)
            {
                ...
                foreach (var context in contexts)
                {
                    ...
                    if (agent == null || agent.Type != AgentType.Enemy) continue;
            ...
            else
            {
                ...
                var agent = damageable.GetBaseAgent();
                if (agent == null || agent.Type != AgentType.Enemy) return;
        ...
        {
            AddTriggerInstance(new BaseDamageableWrapper(target.Cast<Enemies.EnemyAgent>().Damage.Cast<IDamageable>()), mod);
        }
        ...
        private void AddTriggerInstance(BaseDamageableWrapper wrapper, float triggerAmt)
        {
            if (!_storedDebuffs.TryGetValue(wrapper, out var debuff))
            {
                ...
                debuff.modifier = DebuffManager.AddArmorShredDebuff(wrapper.Object!, 1f, DebuffID);
